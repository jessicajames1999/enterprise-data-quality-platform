...
def send_alert_and_trigger_github(error_details):
    """Send PagerDuty alert AND trigger GitHub Action with incident ID"""
    ...
    if pagerduty_response:
        ...
        print("PagerDuty alert failed, but continuing with GitHub trigger")
    
    # Step 2: Trigger GitHub Action with incident details
    try:
        github_token = Variable.get("github_personal_token")
        github_repo = Variable.get("github_repo")  # format: "username/repo-name"
        
        github_payload = {
            "event_type": "airflow-failure",
            "client_payload": {
                "pipeline_failure": error_details,
        ...
        if github_response.status_code == 204:
            print(f"GitHub Action triggered successfully with incident key: {incident_key}")
        else:
            print(f"GitHub Action trigger failed: {github_response.status_code}")
...
def validate_transformed_tables(**context):
    ...
    if overall_passed:
        ...
        # Send PagerDuty alert AND trigger GitHub Action on failure
        error_details = {
        ...
        # This now sends both PagerDuty alert AND triggers GitHub Action
        incident_key = send_alert_and_trigger_github(error_details)
...
# Task 4: Validate transformed data (with no retries to avoid duplicate alerts)
